...
const Exterminate = () => {
    ...
    const onSubmit = async (data) => {
        ...
        try {
            await saphira.removePresenceFromUser(data.lectureId, data.document);
            setValue('document', ''); 
            
            await showFeedbackAlert('success', `Presença removida para ${data.document}`, 'A operação foi um sucesso!');
        } catch (err) {
            const errorMessage = err.response?.data?.talk 
                ? "Palestra não encontrada" 
                : (err.response?.data?.detail || "Ocorreu um erro desconhecido");
            
            await showFeedbackAlert('error', 'Falha na remoção!', errorMessage);
        } finally {      
            setIsLoading(false); 
            setTimeout(() => {
                setFocus('document');
    ...
    return (
        <>
            <Meta title='COSSI 2026 | Remover presença' />
...
const customSelectStyles = {
    ...
    option: (provided, state) => ({
        ...
        backgroundColor: state.isFocused 
            ? 'var(--state-layers-neutrals-primary-012)'
            : 'transparent',
        cursor: 'pointer',
        padding: '0.75rem',
...
const HideableSection = styled.div`
    ...
    button {
        background-color: var(--content-accent-red);
        margin-top: 1rem;
    }
`
